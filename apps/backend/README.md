...
Esta aplicación vive en `apps/backend`, dentro del monorepo del proyecto.

## Instalación

Las dependencias se instalan desde la raíz del monorepo, que es donde npm monta el `node_modules` compartido:
...
```bash
npm run dev:api     # desarrollo (nodemon)
